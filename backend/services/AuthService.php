...
class AuthService
{
    ...
    public function register($data)
    {
        ...
        foreach ($conflicts as $user) {
            // Unverified email match → candidate for update
            if ($user['EMAIL'] === $data['email'] && $user['IS_VERIFIED'] == 0) {
                $sameUnverifiedUser = $user;
            }

            // Employee ID taken by a different verified user → block
            if (
        ...
        // 4. Update unverified account if it exists
        if ($sameUnverifiedUser) {
            $this->user->update($data['email'], [
                ...
                // Role is not updated — keeps whatever was previously set
            ]);
        ...
        // 5. No conflicts → create new user with default role 'Staff'
        $this->user->create([
            'full_name'      => $data['full_name'],
            'email'          => $data['email'],
            'department'     => $data['department'],
    ...
    public function verifyOtp($otpInput)
    {
        ...
        if (
            !$otpInfo ||
            !password_verify($otpInput, $otpInfo['OTP_CODE']) ||
            strtotime($otpInfo['OTP_EXPIRES_AT']) <= time()
        ) {
