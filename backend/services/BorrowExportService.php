...
class BorrowExportService {
  ...
  private static function buildPDFHtml($borrows) {
    ...
    $cancelled = count(array_filter($borrows, fn($b) => $b['STATUS'] === 'Cancelled'));
    ...
    <head>
      ...
      <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .summary-box         { display:table-cell; width:16.66%; padding:8px 12px;
  ...
  private static function buildRows($borrows) {
    ...
    foreach ($borrows as $b) {
      ...
      $due_date           = $b['DUE_DATE']    ?? '';
      $return_status      = '—';
      $diff               = 0; // FIX: initialize before the block so it's always defined
      $return_status_color = '#15803d';
      ...
      $rows .= '
        <tr>
          ...
          <td style="color:' . $return_status_color . ';font-weight:600;">'
