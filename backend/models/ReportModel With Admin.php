...
class ReportModel {
  ...
  // REPORT 5: BORROWING ACTIVITY
  // Filters tbl_borrows.borrow_date
  public function getBorrowingActivity($month = '', $year = '') {
    $filter = $this->monthYearClause('b.borrow_date', $month, $year);
  ...
  public function getAssetUtilization($month = '', $year = '') {
    ...
    $sql = "SELECT d.department_name,
                   ...
                   SUM(CASE WHEN a.status='Maintenance' THEN 1 ELSE 0 END) AS maintenance,
                   ROUND(
                     SUM(CASE WHEN a.status='In Use' THEN 1 ELSE 0 END) * 100.0 / NULLIF(COUNT(*),0),
                     1
                   ) AS utilization_pct
            FROM tbl_assets a
            JOIN tbl_departments d ON a.department_id = d.department_id
    ...
    $stmt2 = $this->conn->prepare($sql2);
    $stmt2->execute();
    $overall = $stmt2->fetch(PDO::FETCH_ASSOC);
