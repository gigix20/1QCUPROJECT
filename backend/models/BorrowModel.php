...
class BorrowModel {
  ...
  public function getAllBorrows() {
    $sql = "SELECT b.borrow_id, b.asset_id, b.department_id,
                   ...
                   d.department_name,
                   d.first_name  AS liable_first,
                   d.middle_name AS liable_middle,
                   d.last_name   AS liable_last,
                   d.suffix      AS liable_suffix
            FROM   tbl_borrows b
...
// ADD BORROW
  public function addBorrow($data) {
    ...
    // ← REMOVE updateAssetStatus here — asset stays Available until admin approves
  }
...
public function approveBorrow($borrow_id) {
  ...
  // Set asset to In Use once approved
...
public function cancelBorrow($borrow_id) {
  // Get asset_id before cancelling
  $borrow = $this->getBorrowById($borrow_id);
  ...
  public function markOverdue() {
    $sql = "UPDATE tbl_borrows
            ...
            AND    TO_DATE(due_date, 'YYYY-MM-DD') < TRUNC(SYSDATE)";
    $stmt = $this->conn->prepare($sql);
    $stmt->execute();
    $this->conn->exec("COMMIT");
  }
